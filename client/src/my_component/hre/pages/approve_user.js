...
function Approve() {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/auth-user/userdetails").then((response) => {
  ...
  const Approve = async (user) => {
    ...
    try {
      const result = await axios.post("http://localhost:3001/auth-user/activate", user);
  ...
  const Reject = async (user) => {
    ...
    try {
      const result = await axios.post("http://localhost:3001/auth-user/reject", user);
      alert(result.data.message);
    } catch (error) {
      console.error(error);
    }
