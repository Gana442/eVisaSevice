...
export const Visa_status_hr = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/visa/approvedVisa").then((response) => {
  ...
  const handleArrivalSubmit = async (e) => {
    ...
    try {
      const result = await axios.post("http://localhost:3001/visa/onsiteArrival", fileData);
  ...
  const handleDepartureSubmit = async (e) => {
    ...
    try {
      const result = await axios.post("http://localhost:3001/visa/onsiteDeparture", fileData);
      alert(result.data.message);
      setCondition2(false);
    } catch (error) {
      console.error(error);
