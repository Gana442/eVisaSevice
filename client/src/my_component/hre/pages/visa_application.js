...
export const Visa_applications_hr = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/application/visaApplications").then((response) => {
  ...
  const Process = async (user) => {
    ...
    try {
      const result = await axios.post(`http://localhost:3001/interview/createInterview/${formId}`);
  ...
  const Reject = async (user) => {
    const formId = user.personalInfo.formId;
    try {
      const result = await axios.post(`http://localhost:3001/application/reject/${formId}`);
      alert(result.data.message);
    } catch (error) {
      console.error(error);
    }
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/application/renewal").then((response) => {
  ...
  const rejectRenewal = async(user) =>{
    const userId = user.userId;
    try {
      const result = await axios.post(`http://localhost:3001/application/rejectRenewal/${userId}`);
      alert(result.data.message);
    } catch (error) {
      console.error(error);
    }
