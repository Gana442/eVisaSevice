...
export const Schedule_interview = ({ user, setAction }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const result = await axios.post("http://localhost:3001/interview/schedule", formData);
    ...
    const Reject = async(e)=>{
        ...
        try {
            const result = await axios.post(`http://localhost:3001/interview/reject/${formId}`);
            alert(result.data.message);
            setAction(false);
        } catch (error) {
            console.error(error);
