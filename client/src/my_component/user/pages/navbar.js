...
export const Dashboard_user = ({ user }) => {
  ...
  useEffect(() => {
    const fetchApplicationData = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/application/ack/${user.userId}`);
  ...
  useEffect(() => {
    if (formId) {
      axios.get(`http://localhost:3001/interview/interviewDetails/${formId}`)
  ...
  useEffect(() => {
    if (formId) {
      axios.get(`http://localhost:3001/visa/approvedVisa/${formId}`).then((response) => {
  ...
  useEffect(() => {
    if (visaNo) {
      axios.get(`http://localhost:3001/visa/arrival/${visaNo}`)
  ...
  useEffect(() => {
    if (visaNo) {
      axios.get(`http://localhost:3001/visa/departure/${visaNo}`)
